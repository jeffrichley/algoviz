...
type RoutingMap = dict[str, list[str]]  # event_type -> handler_names
...
class RoutingRegistry:
    ...
    def register(cls, algorithm: str, routing_map: RoutingMap) -> None:
        ...
        if algorithm in cls._routing_maps:
            raise ValueError(
                f"Routing for algorithm '{algorithm}' is already registered"
            )
...
def validate_routing_map(routing_map: RoutingMap) -> list[str]:
    ...
    for event_type, handlers in routing_map.items():
        ...
        if not isinstance(handlers, list):
            errors.append(
                f"Handlers for '{event_type}' must be list, got {type(handlers)}"
            )
            continue
        ...
        for handler in handlers:
            ...
            if "." not in handler:
                ...
                if len(parts) != 2:
                    ...
                    errors.append(
                        f"Handler '{handler}' has empty widget or action name"
                    )
...
def validate_event_coverage(
    routing_map: RoutingMap, event_types: set[str]
) -> list[str]:
