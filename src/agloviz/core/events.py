...
class VizEvent(BaseModel):
    ...
    payload: dict[str, Any] = Field(
        default_factory=dict, description="Structured metadata (node, pos, weight)"
    )
    step_index: int = Field(..., description="Monotonically increasing step counter")
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Optional metadata (complexity, counters)"
    )
...
def validate_event_payload(event_type: str, payload: dict[str, Any]) -> list[str]:
    ...
    _validate_node_payload(payload, errors)
    _validate_parent_payload(payload, errors)
    _validate_weight_payload(payload, errors)

    return errors


def _validate_node_payload(payload: dict[str, Any], errors: list[str]) -> None:
...
def _validate_parent_payload(payload: dict[str, Any], errors: list[str]) -> None:
...
def _validate_weight_payload(payload: dict[str, Any], errors: list[str]) -> None:
    """Validate WEIGHT payload."""
