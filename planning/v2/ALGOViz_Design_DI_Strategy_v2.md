...
**Version:** v2.0 (Updated for Widget Architecture v2.0 - scene configuration DI)
...
## 13. Widget Registry via DI Factories (Updated for v2.0)
...
**SceneEngine → Director** asks registry for instances per scene configuration.
...
Scene configurations use Hydra-Zen for type-safe widget instantiation:

```python
# scene/pathfinding.py
from hydra_zen import builds, make_config
from agloviz.widgets.structures import ArrayWidget, QueueWidget
from agloviz.widgets.pathfinding import PathfindingGrid

# Create structured configs for widgets
ArrayWidgetConfig = builds(ArrayWidget, populate_full_signature=True)
QueueWidgetConfig = builds(QueueWidget, populate_full_signature=True)
PathfindingGridConfig = builds(PathfindingGrid, populate_full_signature=True)

# Scene configuration with widget specs
@dataclass
class BFSSceneConfig:
    widgets: dict[str, Any] = field(default_factory=lambda: {
        "grid": PathfindingGridConfig(width=20, height=15),
        "queue": QueueWidgetConfig(max_size=100),
        "visited": ArrayWidgetConfig(size=400)
    })
    
    event_bindings: dict[str, list[EventBinding]] = field(default_factory=lambda: {
        "enqueue": [
            EventBinding(widget="queue", action="add_element", params={"element": "${event.node}"}, order=1),
...
SceneEngine uses Hydra to instantiate widgets from scene configurations:
...
class SceneEngine:
    def __init__(self, scene_config: SceneConfig, registry: ComponentRegistry):
        self.scene_config = scene_config
        self.registry = registry
        self.widgets = {}
    
    def initialize_widgets(self) -> None:
        """Instantiate widgets from scene configuration using DI."""
        for widget_name, widget_spec in self.scene_config.widgets.items():
            if hasattr(widget_spec, '_target_'):
                ...
                factory = self.registry.get_factory(widget_spec.widget_type)
                widget_instance = factory(**widget_spec.params)
            
            self.widgets[widget_name] = widget_instance
```

### 14.3 Parameter Template Resolution
Scene configurations support parameter templates resolved at runtime:

```python
# Event binding with parameter templates
event_binding = EventBinding(
    widget="grid",
    action="highlight_element",
    params={
        "index": "${event.pos}",          # Resolved from event data
        "style": "${config.highlight_style}",  # Resolved from configuration
        "duration": "${timing.fast}"      # Resolved from timing config
    }
)
...
## 15. Lifecycle Tuning (Singleton vs Per-run vs Transient)
...
## 18. Developer Experience Enhancements
